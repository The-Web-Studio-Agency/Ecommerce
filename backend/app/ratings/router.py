...
from app.auth.dependencies import CurrentUser, require_admin, require_staff
...
async def get_product_reviews(
    ...
    reviews, _ = await service.list_product_reviews(product_id, tenant.id, page=page, size=size)
    return reviews
...
async def get_product_rating_summary(
    ...
    return await service.get_summary(product_id, tenant.id)
...
async def update_review(
    ...
    return await service.update_review(
        review_id=review_id, user_id=current_user.id, tenant_id=tenant.id, data=data
    )
...
async def delete_review(
    ...
    await service.delete_review(review_id=review_id, user_id=current_user.id, tenant_id=tenant.id)