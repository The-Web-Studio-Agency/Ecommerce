...
class SearchFilterRepository:
    ...
    def _token_matches(token: str):
        ...
        SKU, or gender. Apostrophes are stripped from the stored text the
        same way they were stripped from the token, so a query token like
        "Womens" (from "Women's") still matches text that actually reads
        "Women's"."""
        pattern = f"%{token}%"
