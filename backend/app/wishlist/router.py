...
async def get_wishlist(session:AsyncSession=Depends(get_db),user:User=Depends(require_customer)) ->ApiResponse[WishListRead]:
    wishlist=await _service(session,user).view()
    return ok(wishlist,message="wishlist retrieved successfully")

@router.post("/items",response_model=ApiResponse[WishListRead],status_code=status.HTTP_201_CREATED,summary="add an item to the wishlist")
async def add_item(data:WishListItemCreate,session:AsyncSession=Depends(get_db),user:User=Depends(require_customer)) ->ApiResponse[WishListRead]:
    wishlist=await _service(session,user).add_item(data)
    return ok(wishlist,message="item added to wishlist successfully")

@router.delete("/items/{item_id}",response_model=ApiResponse[WishListRead],summary="remove an item from the wishlist")
async def remove_item(item_id:UUID,session:AsyncSession=Depends(get_db),user:User=Depends(require_customer)) ->ApiResponse[WishListRead]:
...
async def clear_wishlist(session:AsyncSession=Depends(get_db),user:User=Depends(require_customer)) ->ApiResponse[WishListRead]:
